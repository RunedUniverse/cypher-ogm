		...
		stage('Test') {
			parallel {
				...
				stage('Module Neo4J') {
					environment {
						// start Neo4J
						JENKINS_ROGM_TEST_NEO4J_ID= sh (returnStdout: true, script: 'docker run -d --volume=${WORKSPACE}/src/test/resources/neo4j:/var/lib/neo4j/conf --volume=/var/run/neo4j-jenkins-rogm:/run neo4j').trim()
